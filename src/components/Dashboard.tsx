...
import { Plus, Rss, CloudSun, Car, Search, Layers, X } from 'lucide-react';
...
function resolveCascadeGeometries(
  ...
  return (
    <div className="w-full max-w-[1920px] 2xl:max-w-[98%] mx-auto px-4 sm:px-6 pb-12">
      ...
      {/* Single Add (+) Button in Edit Mode */}
      {isEditMode && (
        <div className="flex items-center justify-center mt-10 mb-6">
          <button
            onClick={() => setIsAddPickerOpen(true)}
            className="flex items-center gap-2.5 px-6 py-3 rounded-2xl bg-[var(--color-primary)] text-white hover:opacity-90 active:scale-95 transition-all shadow-lg hover:shadow-xl font-bold text-sm"
            title="Ajouter un élément au portail"
          >
